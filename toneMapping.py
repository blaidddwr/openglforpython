...
void main()
{
    ...
    vec3 tColor = texture(imageTexture,fTexturePoint).xyz;
    vec3 hdr = 0.2*tColor;
    for (int i = 0;i < LIGHT_SIZE;i++)
    {
        hdr += calcPointLight(lights[i],fPosition,fNormal,tColor);
    }
    vec3 mapped = hdr/(hdr+vec3(1));
